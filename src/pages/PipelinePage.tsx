...
import { CalendarClock, ChevronRight, Flame, GripVertical } from 'lucide-react'
...
function PipelineColumn({ status, leads, onOpen }: { status: PipelineStatus; leads: Lead[]; onOpen: (lead: Lead) => void }) {
  const { isOver, setNodeRef } = useDroppable({ id: status })
  const total = leads.reduce((sum, lead) => sum + lead.estimatedValue, 0)
  return <section ref={setNodeRef} className={isOver ? 'kanban-column kanban-column--over' : 'kanban-column'}><header><div><span className={`status-dot status-dot--${status.replaceAll(' ','-').toLowerCase()}`} /><h2>{status === 'Futuro' ? 'Futuro / retomar' : status}</h2><em>{leads.length}</em></div><strong>{currency.format(total)}</strong></header><div className="kanban-column__cards">{leads.map((lead) => <PipelineCard key={lead.id} lead={lead} onOpen={() => onOpen(lead)} />)}{leads.length === 0 && <div className="kanban-empty">Solte um lead aqui</div>}</div></section>
}
...
export function PipelinePage() {
  ...
  const onDragEnd = ({ active, over }: DragEndEvent) => { if (over && columns.includes(over.id as PipelineStatus)) moveLead(String(active.id), over.id as PipelineStatus) }
  return <div className="page page--wide"><section className="page-heading"><div><span className="eyebrow">Jornada comercial</span><h1>Pipeline</h1><p>Arraste os cards ou use o seletor no detalhe para avançar oportunidades.</p></div><div className="pipeline-total"><span>Valor total no funil</span><strong>{currency.format(leads.filter((l) => !['Perdido','Fechado'].includes(l.pipelineStatus)).reduce((sum,l) => sum + l.estimatedValue,0))}</strong></div></section><DndContext sensors={sensors} onDragEnd={onDragEnd}><div className="kanban-board">{columns.map((status) => <PipelineColumn key={status} status={status} leads={leads.filter((lead) => lead.pipelineStatus === status)} onOpen={setSelected} />)}</div></DndContext>
